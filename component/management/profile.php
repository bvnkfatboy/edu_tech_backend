...
// ตรวจสอบว่ามีพารามิเตอร์ GET 'memberID' ที่ถูกส่งมาหรือไม่
if (isset($_GET['memberID'])) {
    // ทำการถอดรหัส Base64 และตรวจสอบค่าที่ถูกต้องหรือไม่
    $decodedMemberID = base64_decode($_GET['memberID']);
    if (is_numeric($decodedMemberID)) {
        $memberID = $decodedMemberID;

        // ทำการ query เพื่อดึงข้อมูลผู้ใช้
        $sql = "SELECT * FROM accounts WHERE acc_id='".$memberID."'";
        $result = mysqli_query($connection, $sql);

        // ตรวจสอบว่ามีข้อมูลที่ต้องการจะดึงออกมาหรือไม่
        if ($result && mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_array($result);
            // ใช้ข้อมูลใน $row ตามต้องการ
            $username = $row['acc_user'];
            $name = $row['acc_name'];
        } else {
            // ถ้าไม่พบข้อมูลผู้ใช้
            // แสดงข้อความแจ้งเตือนและเปลี่ยนเส้นทางไปยังหน้า '?page=member'
            echo '<script>
                Swal.fire({
                    icon: "error",
                    title: "แจ้งเตือน",
        ...
        // ถ้ามีการถอดรหัสไม่สำเร็จหรือมีค่าไม่ถูกต้อง
        // แสดงข้อความแจ้งเตือนและเปลี่ยนเส้นทางไปยังหน้า '?page=member'
        echo '<script>
            Swal.fire({
                icon: "error",
    ...
    // ถ้าไม่มีพารามิเตอร์ GET 'memberID' ที่ถูกส่งมา
    // แสดงข้อความแจ้งเตือนและเปลี่ยนเส้นทางไปยังหน้า '?page=member'
    echo '<script>
        Swal.fire({
            icon: "error",
          ...
          <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form class="space-y-4 md:space-y-6" action="" method="post">
                  <div>
                      ...
                      <input type="text" value="<?=$username; ?>" name="username" id="username" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 light:bg-gray-700 light:border-gray-600 light:placeholder-gray-400 light:text-white light:focus:ring-blue-500 light:focus:border-blue-500"  required="">
                  </div>
                  <div>
                      <label for="text" class="block mb-2 text-sm font-medium text-gray-900 light:text-white">ชื่อ - นามสกุล</label>
                      <input type="text" value="<?=$name; ?>" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 light:bg-gray-700 light:border-gray-600 light:placeholder-gray-400 light:text-white light:focus:ring-blue-500 light:focus:border-blue-500"  required="">
                  </div>
