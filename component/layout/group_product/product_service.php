...
if (isset($_POST['product_service']) && $_POST['product_service'] == "create") {
    ...
    if ($option == 'PC') {
        ...
        if (isset($_FILES['imagepc']) && $_FILES['imagepc']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($file_extension, $allowed_types)) {
                ...
                if (move_uploaded_file($_FILES['imagepc']['tmp_name'], $upload_file)) {
                    ...
                    VALUES ('$mediatitle','$upload_url', 'คอมพิวเตอร์','$medialink', NOW(),'$upload_file','$option_type','product_service')";
        ...
        VALUES ('$mediatitle','$imgonweb', 'เว็บไซต์','$medialink', NOW(),'$option_type','product_service')";
        if ($connection->query($insert_sql) === TRUE) {
            // สำเร็จ
            echo '
            <script>
...
if (isset($_GET['delete_img'])) {
    ...
    $sql = "SELECT * FROM `group_media` WHERE `media_id` = $deleteImgId AND `group` = 'product_service'";
    $result = $connection->query($sql);
    $imgRow = $result->fetch_assoc();

    if ($imgRow && isset($imgRow['media_id']) && $imgRow['media_id'] == $deleteImgId) {
        deleteGroupImageAndData($connection,$deleteImgId,'product_service');
    }
}
?>
...
<div class="p-4 sm:ml-64">
    <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg light:border-gray-700 mt-14">
        ...
        <div class="flex flex-wrap py-5 ">
            <?php $sql = "SELECT * FROM `group_media` WHERE `group` = 'product_service' ";
            $result = $connection->query($sql); ?>
