...
// var_dump($_POST);
if (isset($_POST['product_service']) && $_POST['product_service'] == "create") {
    ...
    if ($option == 'PC') {
        ...
        if (isset($_FILES['imagepc']) && $_FILES['imagepc']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($file_extension, $allowed_types)) {
                ...
                $upload_dir = 'dist/img/product_service/';
                ...
                if (move_uploaded_file($_FILES['imagepc']['tmp_name'], $upload_file)) {
                    ...
                    $insert_sql = "INSERT INTO product_service (media_title , img_resource, img_source,media_link, media_date,img_location,media_type) 
                    ...
                    if ($connection->query($insert_sql) === TRUE) {
                        ...
                        <script>
                            Swal.fire({
                                    ...
                                    window.location.href = "?page=product_service";
                                    }
                            });
                        </script>';
                    } else {
        ...
        $insert_sql = "INSERT INTO product_service (media_title , img_resource, img_source,media_link, media_date,media_type) 
...
if (isset($_GET['delete_img'])) {
    ...
    $sql = "SELECT * FROM `product_service` WHERE `media_id` = $deleteImgId";
    ...
    if ($imgRow && isset($imgRow['media_id']) && $imgRow['media_id'] == $deleteImgId) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = '?page=delete&&deletetype=product_service&&confirm_delete=$deleteImgId';
            }
        });
    </script>";
    }
...
<div class="p-4 sm:ml-64">
    <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg light:border-gray-700 mt-14">
        <button data-modal-target="product_service-create" data-modal-toggle="product_service-create" class="block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
            เพิ่มรูปภาพ
        </button>

        <div class="flex flex-wrap py-5 ">
            <?php $sql = "SELECT * FROM `product_service`";
            $result = $connection->query($sql); ?>
            <?php while ($row = $result->fetch_assoc()) { ?>
                <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 mx-3">
                    ...
                    <div class="p-5 ">
                        ...
                        <a href="?page=product_service&&delete_img=<?php echo $row['media_id']; ?>" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800">
                            <div class="pr-1">
...
<div id="product_service-create" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full" style="max-width: 35rem;">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                ...
                <button type="button" id="product_service-create-close" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="product_service-create">
            ...
            <!-- Modal body -->
            <form class="p-4 md:p-5 product_service-create-body" action="" method="post" enctype="multipart/form-data">
                <div class="grid gap-4 mb-4 grid-cols-2">
                    ...
                    <div class="col-span-2 hidden" id="option-type">
                ...
                <button type="submit" name="product_service" value="create" class="text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const modal = document.getElementById('product_service-create');
        const closeModalBtn = document.getElementById('product_service-create-close');

        closeModalBtn.addEventListener('click', () => {
            // ดำเนินการล้างค่าหรือทำอย่างอื่นตามที่คุณต้องการ
            // ตัวอย่าง: ล้างค่า input fields
            const inputFields = document.querySelectorAll('.product_service-create-body input');
