...
if (isset($_POST['gallery']) && $_POST['gallery'] == "create") {
    ...
    if ($option == 'PC') {
        ...
        if (isset($_FILES['imagepc']) && $_FILES['imagepc']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($file_extension, $allowed_types)) {
                ...
                if (move_uploaded_file($_FILES['imagepc']['tmp_name'], $upload_file)) {
                    ...
                    $insert_sql = "INSERT INTO gallery (img_title , img_resource, img_source,img_link, img_update,img_location) 
                    VALUES ('$titleimg','$upload_url', 'คอมพิวเตอร์','$linkclound', NOW(),'$upload_file')";
        ...
        $insert_sql = "INSERT INTO gallery (img_title ,img_resource, img_source,img_link, img_update) 
        VALUES ('$titleimg','$upload_url','เว็บไซต์','$linkclound', NOW())";
        if ($connection->query($insert_sql) === TRUE) {
            // สำเร็จ
            echo '
            <script>
...
if (isset($_GET['delete_img'])) {
    ...
    $sql = "SELECT * FROM `gallery` WHERE `img_id` = $deleteImgId";
    ...
    if ($imgRow && isset($imgRow['img_id']) && $imgRow['img_id'] == $deleteImgId) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = '?page=delete&&deletetype=gallery&&confirm_delete=$deleteImgId';
            }
        });
    </script>";
    }
...
<div class="p-4 sm:ml-64">
    <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg light:border-gray-700 mt-14">
        <button data-modal-target="gallery-create" data-modal-toggle="gallery-create" class="block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
            เพิ่มรูปภาพ
        </button>
            ...
            <?php while ($row = $result->fetch_assoc()) { ?>
                <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 mx-3">
                    <a href="#" onclick="openModal('<?php echo $row['img_resource']; ?>')">
                        <img class="rounded-lg img_detail_menu" src="<?php echo $row['img_resource']; ?>" alt="<?php echo $row['img_title']; ?>" />
                    </a>
                    <div class="p-5 ">
                        <div class="mb-3 flex flex-wrap justify-between">
                            <p class=" text-xs text-gray-700 dark:text-gray-400">ชื่อภาพ/งาน: <?php echo $row['img_title']; ?></p>
                        ...
                        <a href="<?php echo $row['img_link']; ?>" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-green-700 rounded-lg hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">
                        ...
                        <a href="?page=gallery&&delete_img=<?php echo $row['img_id']; ?>" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800">
...
<!-- gallery create  -->
<div id="gallery-create" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full" style="max-width: 35rem;">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                ...
                <button type="button" id="gallery-create-close" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="gallery-create">
            ...
            <!-- Modal body -->
            <form class="p-4 md:p-5 gallery-create-body" action="" method="post" enctype="multipart/form-data">
                <div class="grid gap-4 mb-4 grid-cols-2">
                    <div class="col-span-2" >
                        <label for="titleimg" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">ชื่อภาพ หรือ ชื่องาน</label>
                        <input type="text" name="titleimg" id="titleimg" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="" required>
                    ...
                    <div class="col-span-2" >
                        <label for="linkclound" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">ลิ้งก์เชื่อมโยงไปยังคลังภาพ</label>
                        <input type="text" name="linkclound" id="linkclound" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="" value="#">
                        <label  class="hidden mt-5 flex flex-col items-center justify-center w-full  border-2 border-gray-300 border-dashed rounded-lg  bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6"></div>
                        </label>
                    </div>
                ...
                <button type="submit" name="gallery" value="create" class="text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const modal = document.getElementById('gallery-create');
        const closeModalBtn = document.getElementById('gallery-create-close');

        closeModalBtn.addEventListener('click', () => {
            // ดำเนินการล้างค่าหรือทำอย่างอื่นตามที่คุณต้องการ
            // ตัวอย่าง: ล้างค่า input fields
            const inputFields = document.querySelectorAll('.gallery-create-body input');
